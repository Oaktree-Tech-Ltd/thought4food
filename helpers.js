    ...
    console.log(
        {
            ...
            "carbohydrate": `${result['carbohydrate']} (${percentage(total, result['carbohydrate'])}%)`,
...
const getNutrition = (plan) => Object.keys(plan)
    .map(k => plan[k])
    .reduce((acc, v) => ([...acc, ...v]), [])
    .reduce(({calories, protein, carbohydrate, fat}, v) => {
        if(v.split(':').length == 1) {
